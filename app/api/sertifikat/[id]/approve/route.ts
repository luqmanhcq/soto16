...
import { approveSertifikatSchema } from '@/lib/validations/sertifikat.validation'
...
/**
 * POST /api/sertifikat/[id]/approve - Admin only
 */
export async function POST(
    ...
    try {
        ...
        const validatedData = approveSertifikatSchema.parse(body)
        const approved = await sertifikatService.approve(idNum, user.id, validatedData)
        return successResponse(approved, 'Usulan sertifikat berhasil disetujui')
    } catch (error) {
